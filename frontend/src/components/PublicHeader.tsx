...
export function PublicHeader() {
  ...
  const footer = status.kind === 'authenticated'
    ? <Space spacing="tight" className="public-header-actions"><Button theme="borderless" aria-label={nextLanguage === 'zh-CN' ? '中文' : 'English'} onClick={() => setStoredLanguage(nextLanguage)}>{nextLanguage === 'zh-CN' ? '中文' : 'EN'}</Button><Button theme="solid" type="primary" onClick={() => navigate('/console/dashboard')}>{t('nav.console')}</Button><div className="public-account"><Avatar size="small" className="public-avatar" aria-label={t('auth.avatarLabel', { username: account?.username ?? '' })}>{account?.username.charAt(0).toUpperCase()}</Avatar><span className="public-username">{account?.username}</span><Button theme="borderless" className="public-logout" onClick={() => void handleSignOut()}>{t('auth.signOut')}</Button></div></Space>
